...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::table('inventory_transactions', function (Blueprint $table) {
            ...
            $table->foreignId('unit_id')->nullable()->after('notes')->constrained('product_units')->onDelete(
                'set null'
            );
            $table->integer('unit_quantity')->nullable()->after('unit_id')->comment('Quantity in the specified unit');
            $table->boolean('is_adjustment')->default(false)->after('unit_quantity')->comment(
                'Whether this is an inventory adjustment'
            );
        });
    }
